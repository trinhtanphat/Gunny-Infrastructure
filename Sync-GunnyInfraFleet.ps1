...
if(-not(Test-Path (Join-Path $PSScriptRoot 'lib'))){throw 'Bundle lib directory missing.'}
foreach($s in $servers){
  ...
  try{
    ...
    if($local){
      New-Item -ItemType Directory -Force -Path $infraRoot,(Join-Path $infraRoot 'lib')|Out-Null
      ...
      Invoke-Command -Session $session -ScriptBlock {param($r) New-Item -ItemType Directory -Force -Path $r,(Join-Path $r 'lib')|Out-Null} -ArgumentList $infraRoot
